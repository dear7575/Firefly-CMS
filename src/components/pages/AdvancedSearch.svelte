<script lang="ts">
	...
	// 获取搜索方法配置
	const searchMethod =
		navBarSearchConfig.method || NavBarSearchMethod.PageFind;
	...
	onMount(() => {
		...
		// 使用后端 API 或开发环境直接初始化
		if (
			searchMethod === NavBarSearchMethod.BackendAPI ||
			import.meta.env.DEV
		) {
			initialize();
		} else {
...
<div
	class="card-base px-6 py-6 md:px-9 md:py-6 mb-4 rounded-[var(--radius-large)]"
>
	<!-- Title Section -->
	<div class="mb-4">
...
<div class="grid grid-cols-1 gap-4">
	...
	<div>
		...
		{:else if results.length > 0}
			<div class="space-y-4">
				{#each results as result}
					<div
						class="card-base p-6 block rounded-[var(--radius-large)]"
					>
						<a href={result.url} class="block group">
							<h5
								class="mb-2 text-2xl font-bold tracking-tight text-90 group-hover:text-[var(--primary)] transition-colors"
							>
		...
		{:else if keyword}
			<div
				class="card-base p-10 text-center text-50 rounded-[var(--radius-large)]"
			>
